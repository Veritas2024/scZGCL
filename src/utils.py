...
def init_log(name):
    ...
    consoleFormatter.converter = beijing
    fileFormatter = logging.Formatter('%(asctime)s || %(filename)s:%(message)s',datefmt='%Y-%m-%d %H:%M:%S')
    fileFormatter.converter = beijing

    consoleHander.setFormatter(consoleFormatter)
